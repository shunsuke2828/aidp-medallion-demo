...
## Why Modern Data Platforms Are Needed

企業内のデータ活用は、以前よりも複雑になっています。業務システム、ログ、Web行動、外部データなど、データの種類と量が増え、BIだけでなくAI/ML、データ共有、ガバナンスにも同じデータを使いたい場面が増えています。

従来型の構成では、用途ごとにデータウェアハウス、データレイク、ETLツール、分析基盤、機械学習環境が分かれがちでした。その結果、同じデータを何度もコピーしたり、どれが正しいデータか分からなくなったり、品質チェックや権限管理が分散したりします。
...
まず使うファイルは [`aidp_medallion_demo_cells.ipynb`](./aidp_medallion_demo_cells.ipynb) です。AIDPにこのNotebookをインポートし、Catalog名を自分の環境に合わせて変更してから上から順番に実行します。

| ファイル | 用途 |
|---|---|
| [`aidp_medallion_demo_cells.ipynb`](./aidp_medallion_demo_cells.ipynb) | AIDPへインポートして実行するNotebook |
| [`aidp_medallion_demo_cells.md`](./aidp_medallion_demo_cells.md) | NotebookセルをMarkdownで確認・コピーするためのファイル |
| [`AIDP_Medallion_Demo_Runbook.md`](./AIDP_Medallion_Demo_Runbook.md) | AIDPで一から実行するための手順書 |
| [`AIDP_Medallion_Demo_Runbook.docx`](./AIDP_Medallion_Demo_Runbook.docx) | 共有用のWord版手順書 |
| [`aidp_medallion_demo_queries_and_cleanup.sql`](./aidp_medallion_demo_queries_and_cleanup.sql) | 追加確認SQLとクリーンアップSQL |
| [`aidp_medallion_data_transformation.svg`](./aidp_medallion_data_transformation.svg) | データ変換イメージ図 |
| [`aidp_medallion_data_transformation_preview.png`](./aidp_medallion_data_transformation_preview.png) | README表示用のプレビュー画像 |
...
- NotebookにSpark Computeがアタッチされていること
- `CATALOG = "<your_catalog>"` をAIDP UIで作成したCatalog名に変更していること
- Schema `production` が作成済みであること
- Managed Volume `demo_raw_landing` と `demo_artifacts` が作成済みであること
- Volume POSIXパス `/Volumes/<your_catalog>/production/...` をNotebookから参照できること
