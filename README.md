...
GitHubへ公開する場合は、秘密情報が混ざっていないことを確認してからpushしてください。

```bash
git status
git add README.md .gitignore \
  aidp_medallion_demo_cells.ipynb \
  aidp_medallion_demo_cells.md \
  AIDP_Medallion_Demo_Runbook.md \
  AIDP_Medallion_Demo_Runbook.docx \
  aidp_medallion_demo_queries_and_cleanup.sql \
  aidp_medallion_data_transformation.svg \
  aidp_medallion_data_transformation_preview.png
git commit -m "Add AIDP medallion architecture demo"
```

GitHub CLIを使う場合:

```bash
gh repo create aidp-medallion-demo --public --source=. --remote=origin --push
```
